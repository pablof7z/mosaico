...
/// store and emit a `Leave` tail event for each `(pubkey, channel)`
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let still_alive: std::collections::HashSet<(String, String)> = state.with_store(|s| {
                tracked_keys
                    .iter()
                    .filter(|(pubkey, channel)| {
                        s.get_status(pubkey, channel)
                            .ok()
                            .flatten()
            ...
            let to_leave: Vec<(String, PeerTracked)> = {
                ...
                for key in expired {
                    if let Some(tracked) = map.remove(&key) {
                        leaves.push((key.0, tracked));
                    }
                }
                leaves
            };
            for (pubkey, tracked) in to_leave {

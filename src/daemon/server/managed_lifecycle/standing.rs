use super::publish_fence::SessionPublishFence;
use super::*;
...
pub(in crate::daemon::server) async fn lock_session_route_for_publish<'a>(
    ...
    // We reached this branch because the current NMP snapshot does not
    ...
    Ok(SessionPublishFence {
        _lane: lane,
        confirmed_scope: Some(ConfirmedGroupScope::from_nmp_membership(
            channel,
            &session.pubkey,
        )),
    })

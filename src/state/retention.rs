...
impl RetentionPruneReport {
    pub fn total(self) -> usize {
        self.delivered_inbox + self.completed_event_claims + self.native_turn_attempts
    }
}

impl Store {
    pub fn prune_retained_state(&self, now: u64) -> Result<RetentionPruneReport> {
        self.prune_retained_state_before(now.saturating_sub(COMPLETED_LEDGER_RETENTION_SECS))
    }
    ...
    pub fn prune_retained_state_before(
        ...
        let delivered_inbox = self.conn.execute(
            "DELETE FROM inbox
             WHERE state IN (
                 'delivered', 'submitted', 'injected', 'echo_consumed'
             )

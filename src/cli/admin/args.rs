...
/// (`--session @agent-sessionCode <channel>`). Session mode takes ONE positional
...
pub(in crate::cli) struct AddArgs {
    ...
    /// Pull an existing session, named `@agent-sessionCode`, into the channel.

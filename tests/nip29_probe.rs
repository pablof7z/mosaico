...
async fn publish(client: &Client, signed: &Event, label: &str) -> bool {
...
async fn fetch(client: &Client, filter: Filter, label: &str) -> Vec<Event> {
    let evs = client
    ...
    for (attempt, backoff_s) in [2u64, 5, 12].into_iter().enumerate() {
        ...
        match admin_c.send_event(&create).await {
            ...
            Ok(out) if out.failed.values().any(|m| m.contains("rate-limited")) => {
                eprintln!("[probe] 9007 create-group rate-limited (attempt {}); backing off {backoff_s}s", attempt + 1);

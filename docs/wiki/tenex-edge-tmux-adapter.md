...
`tenex-edge who` displays spawnable agents (configured but idle, with no live session) alongside live agents, and the same list is injected into the first-turn context via `push_turn_fabric_block`. The `tenex-edge tmux` command (bare, no subcommand) opens an interactive TUI built with crossterm, showing live sessions with CPU usage and status, spawnable agents, and keybindings for attach (a), new session (n), and quit (q), with 2-second auto-refresh and RAII terminal restore on exit. Help text displays `[↵] attach/spawn`. From the TUI the user can attach to a remote tmux pane to view and interact with the running agent, or spawn a new session with any local agent. Non-attachable live rows do not display the `[no tmux]` tag. Sessions are grouped by project, displayed in switchable tabs (Left/Right arrows) including an 'All' tab as the default. In the 'All' tab, session labels use the format 'slug@project' so the project of each session is identifiable. Exited sessions are hidden by default and require pressing 'e' to toggle visibility. The label 'Spawnable (no session)' is renamed to 'Agents', and per-harness labels like '[spawnable via claude]' are shortened to '[claude]'. Agents appear in all project tabs since they are cross-project. Spawning a new tmux session from a selected project tab creates the session in that project's directory, using the active tab's project filter to resolve the project, and only falls back to current-working-directory resolution when on the 'All' tab where no filter exists. When attached to a session, a persistent sidebar is injected as a narrow left pane (40 columns wide) inside each agent's own tmux window, running `tenex-edge tmux sidebar` and showing all sessions in the same project with the current session highlighted. The sidebar width is re-pinned on every client resize via a session-scoped client-resized hook. `ensure_sidebar` is idempotent: it checks `tmux list-panes` for an existing sidebar pane before splitting the window, and silently swallows errors if one already exists. In the sidebar, ↑/↓ or j/k move selection, q/Esc/Ctrl-c exits the sidebar cleanly. Alt-s moves focus from the agent pane to the sidebar; Alt-a moves focus back to the agent pane, injected by `bind_sidebar_keys` on attach. Pressing Enter on a session in the sidebar calls `ensure_sidebar` on that target session and then runs `tmux switch-client` to switch to it, keeping the sidebar visible across switches. The popup session switcher uses the Ctrl-b s keybinding (tmux prefix + s) instead of Alt-t to avoid macOS terminal option-key encoding issues. In the popup session switcher, selecting a session performs `switch-client` on the underlying client plus `ensure_sidebar` on the target, then exits to close the popup (rather than attaching inline inside the popup); pressing Escape or q dismisses the popup without switching. The popup binding is injected via `inject_popup_binding()` which runs `tmux bind-key -T root M-t display-popup -w 90% -h 80% -E 'tenex-edge tmux'` and silently swallows errors to be unconditionally idempotent. `inject_popup_binding()` is called as the first line of both `attach_pane_blocking()` (the TUI path) and `attach_pane()` (the one-shot CLI path). The persistent sidebar is the intended long-term approach for session switching, replacing the popup approach currently being evaluated. Attach is best-effort: if the tmux pane no longer exists or attaching fails, the TUI transparently falls back to resuming the session (as if it were never attached to tmux) and attaches to the fresh pane, surfacing an error only if resume itself also fails. A `PendingAttach` struct carries the pane ID plus a `resume_sid` fallback session ID; freshly spawned panes have `resume_sid` set to `None`. All four `pending_attach` sites (live attach, spawn, Enter-resume, and r-resume) set `pending_attach` to `Some(pane)` with the session ID in scope.

<!-- citations: [^15629-49] [^15629-55] [^656e1-1] [^9bab9-3] [^9bab9-5] [^9bab9-8] [^9bab9-9] [^9c78b-2] [^a7c75-3] -->

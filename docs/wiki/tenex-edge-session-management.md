...
topic: tenex-edge
summary: The MVP (M1) launches a session via `tenex-edge session-start --agent <agent-slug>`, which forks a background process, creates a session ID, and publishes a pre
tags:
  - capture
volatility: warm
confidence: medium

...
This guide governs the family of `tenex-edge` agent skills written to `./skills/tenex-edge/` with symlinks from `~/.agents/skills/tenex-edge` and `~/.claude/skills/tenex-edge` pointing to the repo-local skill directory. The original skill taught agent operator commands (`who`, `chat`, `tail`, project groups, subgroup rooms, install, and keystore commands), but it has gone stale: self-identity now lives in `who` agent-context output, and targeting uses agent labels like `haiku` / `haiku1`. The current guide restructures that material into a set of focused skills, each kept short with volatile details in `reference/` files.

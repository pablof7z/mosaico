...
| BDD-17 | a user-authored kind:9 p-tags an offline local session pubkey | the exact session has a native resume locator, or is non-resumable | the daemon resumes and injects into the same pubkey; a non-resumable specialist stays pending without minting a sibling | `cargo test --test daemon_integration operator_kind9_to_offline -- --test-threads=1` |
| BDD-18 | a `perSessionKey:false` agent has no live runtime | a user-authored kind:9 p-tags its configured stable pubkey | the daemon starts a runtime under that same stable pubkey and injects the message | `cargo test --test daemon_integration operator_kind9_to_stable_agent_starts_the_same_pubkey -- --test-threads=1` |
| BDD-19 | exact session targeting is needed for chat/channel operations | `--session <session-id>` is supplied | the requested session anchor wins over ambient environment hints | `cargo test channel_send_accepts_explicit_session_anchor` |
